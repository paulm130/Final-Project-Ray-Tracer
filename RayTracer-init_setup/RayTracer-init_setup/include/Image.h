...
class Image {
   ...
   private:
    ...
    void validate_color_pipeline_invariants() const {
        auto near = [](float a, float b, float eps = 1e-5f) {
        ...
        assert(near(srgb_zero.x, 0.0f) && near(srgb_zero.y, 0.0f) && near(srgb_zero.z, 0.0f));
        assert(near(srgb_one.x, 1.0f) && near(srgb_one.y, 1.0f) && near(srgb_one.z, 1.0f));
        assert(near(linear_zero.x, 0.0f) && near(linear_zero.y, 0.0f) && near(linear_zero.z, 0.0f));
        assert(near(linear_one.x, 1.0f) && near(linear_one.y, 1.0f) && near(linear_one.z, 1.0f));
        ...
        assert(near(stress.x, 0.0f) && near(stress.y, 0.0f));
        assert(near(stress.z, 1.0f));
    }
#endif
};
